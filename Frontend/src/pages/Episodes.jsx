...
export default function Episodes() {
    ...
    useEffect(() => {
        if (selectedEpisode?.episodeId && selectedServer.server && selectedServer.type) {
            dispatch(fetchEpisodesStreamLink({
    ...
    console.log(EpisodeStreamLinks?.data?.data)
    ...
    return (<div className="min-h-screen bg-black text-white font-['Crunchyroll_Atyp',_sans-serif] pt-16">
                ...
                <div className="flex items-center gap-4 text-gray-400 text-sm">
                    <span className="flex items-center gap-1">
                        <FaEye className="text-[#f47521]"
                            size={16}/> {totalEpisodes}
                        Episodes
                    </span>
                    {
                    selectedEpisode && (<span className="bg-gray-800 px-2 py-1 rounded text-xs">
                        Episode {
                ...
                <div className="bg-gray-900/90 border border-gray-700 rounded-lg overflow-hidden">
                    <div className="relative w-full bg-gray-800" style={{ aspectRatio: '16/9' }}>
                        {selectedServer.server && EpisodeStreamLinks?.data?.data ? (
                            <VideoPlayer 
                                streamData={EpisodeStreamLinks.data.data}
                                onReady={(player) => {
