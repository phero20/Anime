...
useEffect(() => {
    ...
    >
      {promotionalVideos.map((vid, idx) => (
        ...
        >
          ...
          {/* Title below image */}
          <p className="mt-2 text-sm truncate">{vid.title}</p>
        </a>
      ))}
    </div>
  </div>
                ...
                >
                  ...
                  <div className="flex items-center gap-3 w-1/2">
                    ...
                    <div className="space-y-0.5">
                      <p className="text-sm font-semibold">{pair.character?.name}</p>
                  ...
                  <div className="flex items-center gap-3 w-1/2">
                    ...
                    />
                    <div className="space-y-0.5">
                      <p className="text-sm font-semibold">{pair.voiceActor?.name}</p>
                      <p className="text-xs text-gray-400">{pair.voiceActor?.cast}</p>
                    </div>
                  </div>
                </div>
              ))}
  ...
  <div data-aos="zoom-out-up" className="relative">
    ...
    {showLeftS && (
      <button
        onClick={() => seasonRef.current.scrollBy({ left: -800, behavior: "smooth" })}
        className="hidden md:flex absolute -left-10 top-1/2 -translate-y-1/2 hover:bg-black p-2 rounded-full z-20"
      >
        <FaChevronLeft />
      </button>
    ...
    >
      {seasons.map((s, idx) => (
        <div
          key={idx}
          className={`relative group snap-start min-w-[180px] sm:min-w-[220px] lg:min-w-[260px] ${
            s.isCurrent ? "ring-2 ring-[#f47521]" : ""
          }`}
        >
          <img
            src={s.poster}
          ...
          <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition flex items-center justify-center p-4 text-center">
            <p className="text-sm leading-snug">{s.title || s.name}</p>
          </div>

          <p className="mt-2 text-sm truncate px-1">
            {s.name}
            {s.isCurrent && (
              <span className="text-[#f47521] font-semibold"> • Current</span>
            )}
          </p>
        </div>
      ))}
