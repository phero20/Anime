...
export default function AnimeCards({ data, name, scroll, fetchMoreData, hasMore }) {
    ...
    <Link to={`/anime/${item.id}`} key={index}>
      <div
        ...
        className={`group cursor-pointer relative overflow-hidden snap-start flex-shrink-0 hover:shadow-xl transition-shadow ${
          scroll ? "min-w-[140px] sm:min-w-[160px] md:min-w-[180px] lg:min-w-[200px] xl:min-w-[220px]" : "w-full"
        }`}
      >
        <div className="relative overflow-hidden">
          <img
            src={item.poster}
            alt={item.name}
            className="w-full h-[200px] sm:h-[240px] md:h-[280px] lg:h-[320px] xl:h-[360px] rounded-lg object-cover"
            loading="lazy"
          />
          ...
          <div className="absolute inset-0 bg-gray-900/90 h-[200px] sm:h-[240px] md:h-[280px] lg:h-[320px] xl:h-[360px] rounded-md opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col p-2 sm:p-3 space-y-2">
            ...
            {item.rating && (
              <span className="text-xs sm:text-sm text-gray-200">Rating: {item.rating}</span>
            )}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="w-full max-w-[95%] mx-auto px-2 sm:px-4 md:px-6 py-6 sm:py-8 text-white font-['Crunchyroll_Atyp',_sans-serif] relative">
      ...
      {scroll ? (
        ...
        <InfiniteScroll
          ...
          className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-7 gap-3 sm:gap-4 md:gap-6 scrollbar-hide"
        >
          {data.map(renderCard)}
        </InfiniteScroll>
      )}
