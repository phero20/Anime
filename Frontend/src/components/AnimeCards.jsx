...
export default function AnimeCards({data, name}) {
    ...
    if (!Array.isArray(data) || data.length === 0) {
        return (
            <div className="text-center text-gray-400 py-8">No {name}
                found.</div>
        );
    }
    ...
    return (
        <div className="max-w-screen-2xl mx-auto px-4 md:px-10 py-8 text-white font-['Crunchyroll_Atyp',_sans-serif] relative">
            ...
            <div ref={scrollRef}
                ...
                data.map((item, index) => (
                    <div key={index}
                        className="group cursor-pointer relative rounded-lg overflow-hidden min-w-[160px] sm:min-w-[200px] lg:min-w-[240px] snap-start flex-shrink-0 hover:shadow-xl transition-shadow w-[260px]">
                        ...
                        <div className="relative rounded-md overflow-hidden">
                            ...
                            <img src={
                                ...
                                className="w-full h-[390px] rounded-md object-cover"/> {/* Bottom Title Info */}
                            ...
                            <div className="absolute inset-0 bg-gray-950/90 rounded-md opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col p-3 space-y-2">
                                ...
                                {
                                item.rating && <span className="text-sm text-gray-200">Rating: {
                                    item.rating
                                }</span>
                            } </div>
                        </div>
