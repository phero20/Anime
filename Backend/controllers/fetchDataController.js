...
export const proxyStream = async (req, res) => {
    try {
        ...
        const isM3U8 = url.includes('.m3u8') || response.headers['content-type']?.includes('application/vnd.apple.mpegurl') || response.headers['content-type']?.includes('application/x-mpegURL');
        ...
        if (isM3U8) {
            ...
            response.data.on('end', () => {
                ...
                const rewrittenContent = data.replace(/^(?!https?:\/\/)([^\r\n]+\.m3u8)$/gm, (match, relativePath) => {
                    const fullUrl = baseUrl + relativePath;
                    return `${proxyBaseUrl}/api/anime/proxy-stream?url=${encodeURIComponent(fullUrl)}`;
                }).replace(/^(?!https?:\/\/)([^\r\n]+\.ts)$/gm, (match, relativePath) => {
                    const fullUrl = baseUrl + relativePath;
                    return `${proxyBaseUrl}/api/anime/proxy-stream?url=${encodeURIComponent(fullUrl)}`;
                });
            ...
            response.data.on('error', (streamError) => {
                ...
                if (!res.headersSent) {
                    res.status(500).json({ error: 'M3U8 processing error', details: streamError.message });
                }
            });
        } else {
...
export const fetchEpisodeStreamLinks = async (req, res) => {
    try {
        console.log("running")
        const { episodeId, server, category } = req.body;

        // Check if stream data already exists in database
        const existingStream = await StreamLink.findOne({ episodeId, server, category });

        if (existingStream) {
            console.log('Stream data found in cache for:', episodeId);
            ...
            return res.json({ success: true, data: { data: { ...streamObj, sources: proxiedSources } } });
        }

        // Fetch from external API if not cached
        const response = await axios.get(`${backendUrl}/api/v2/hianime/episode/sources`, {
            params: { animeEpisodeId: episodeId, server, category }
        });

        console.log('Fetched new stream data for:', episodeId);
        ...
        console.log('Stream data cached successfully for:', episodeId);

        // Rewrite sources URLs to go through your HLS proxy
        const proxiedSources = rewriteSourcesToProxy(response.data.data.sources);
        console.log("proxy sources", proxiedSources)
        return res.json({ success: true, data: { data: { ...response.data.data, sources: proxiedSources } } });

    } catch (error) {
        console.error('Error in fetchEpisodeStreamLinks:', error.message);
        res.json({ success: false, message: error.message });
    }
};
